...
struct QuickJsSigner::Impl {
    ...
    std::string eval_str(const std::string& code, const char* label) {
        // Reset the stack base to the current point so the overflow check only
        // measures this eval's own recursion (Go goroutine stacks can already
        // be grown when cgo enters here, which otherwise trips a false
        // "stack overflow").
        JS_UpdateStackTop(rt);
        JSValue r = JS_Eval(ctx, code.c_str(), code.size(), label, JS_EVAL_TYPE_GLOBAL);
        if (JS_IsException(r)) {
            JSValue ex = JS_GetException(ctx);
            std::string msg = "(unknown JS exception)";
            const char* s = JS_ToCString(ctx, ex);
            if (s) { msg = s; JS_FreeCString(ctx, s); }
            JSValue msgProp = JS_GetPropertyStr(ctx, ex, "message");
            if (!JS_IsUndefined(msgProp) && !JS_IsException(msgProp)) {
                const char* ms = JS_ToCString(ctx, msgProp);
                if (ms) { if (msg == "(unknown JS exception)") msg = ms; JS_FreeCString(ctx, ms); }
            }
            JS_FreeValue(ctx, msgProp);
            JSValue stackProp = JS_GetPropertyStr(ctx, ex, "stack");
            if (!JS_IsUndefined(stackProp) && !JS_IsException(stackProp)) {
                const char* ss = JS_ToCString(ctx, stackProp);
                if (ss) { msg += " | " + std::string(ss); JS_FreeCString(ctx, ss); }
            }
            JS_FreeValue(ctx, stackProp);
            JS_FreeValue(ctx, ex);
            JS_FreeValue(ctx, r);
            throw std::runtime_error(std::string("QuickJsSigner ") + label + ": " + msg);
        }
...
void QuickJsSigner::set_user_agent(const std::string& ua) {
    ensure_initialized();
    std::string q = json_quote(ua);
    impl_->eval(
        "try{navigator.userAgent=" + q + ";}catch(e){}"
        "try{globalThis.navigator.userAgent=" + q + ";}catch(e){}"
        "try{window.navigator.userAgent=" + q + ";}catch(e){}",
        "set-user-agent", /*throw_on_exc=*/false);
}
...
std::string QuickJsSigner::sign(const std::string& url) {
    ...
    // Flush microtasks + deferred timeouts the SDK queued during signing.
    // Without this, the SDK's pending jobs accumulate across sign() calls and
    // eventually overflow the JS stack.
    impl_->drain();
    if (signed_url.empty()) {
